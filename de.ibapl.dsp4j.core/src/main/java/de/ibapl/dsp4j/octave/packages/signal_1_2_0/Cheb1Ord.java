 ...
 * cheb1ord(Wp, Ws, Rp, Rs) ## Low pass (Wp<Ws) or high pass (Wp>Ws) filter
 ...
 * Wc] = cheb1ord([Wp1, Wp2], [Ws1, Ws2], Rp, Rs) ## Band pass (Ws1<Wp1<Wp2<Ws2)
 * or band reject (Wp1<Ws1<Ws2<Wp2) ## filter design. Wp gives the edges of the
 * pass band, and Ws gives ## the edges of the stop band. ## ## See also: cheby1
 */
public class Cheb1Ord extends IirFilterOrder {
    ...
    private void calcCheb1Ord(double[] Wp, double[] Ws, double Rp, double Rs) {
        ...
        n = (int) Math.ceil(FastMath.acosh(Math.sqrt((stop_atten - 1.0) / (pass_atten - 1.0))) / FastMath.acosh(Wa));

    }

    public Cheb1Ord(double wp, double ws, double rp, double rs) {
        if (wp < ws) {
            stop = false;
        } else if (wp > ws) {
            stop = true;
        ...
        calcCheb1Ord(new double[]{wp}, new double[]{ws}, rp, rs);
    }

    public Cheb1Ord(double wpl, double wph, double wsl, double wsh, double rp, double rs) {
        ...
        calcCheb1Ord(new double[]{wpl, wph}, new double[]{wsl, wsh}, rp, rs);
    }
