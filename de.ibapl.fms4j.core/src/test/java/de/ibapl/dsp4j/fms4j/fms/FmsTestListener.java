...
public class FmsTestListener implements FmsContainerListener {
    ...
    public void success(FmsData fmsData) {
        assertEquals(data.get(currentDataIndex),
                fmsData,
                String.format("Index %d\n%s\n%s\n", currentDataIndex,
                        data.get(currentDataIndex).toCsvString(), fmsData.toCsvString()));
        currentDataIndex++;
    }
    ...
    public boolean txtCharParityError(FmsData fmsData, String txt) {
        assertEquals(data.get(currentDataIndex),
                ...
                String.format("Index %d\n%s\n%s\nTXT:%s\n", currentDataIndex,
                        data.get(currentDataIndex).toCsvString(), fmsData.toCsvString(), txt));
        currentDataIndex++;
    ...
    public void doTest(String dirName, String resFileName) throws Exception {
        ...
        ShortSampledSource msfs = new ShortSampledSource(
                new BufferedInputStream(FmsTestListener.class.getResourceAsStream(String.format("/%s/%s", dirName, resFileName))), 1);
        ...
        assertEquals(data.size(), currentDataIndex);
    }
