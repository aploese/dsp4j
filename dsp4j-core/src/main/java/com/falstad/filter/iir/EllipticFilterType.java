...
public abstract class EllipticFilterType extends PoleFilterType {
    ...
    void selectElliptic(int s, Scrollbar[] auxBars, Label[] auxLabels) {
    ...
    void setupElliptic(int a, Scrollbar[] auxBars, Label[] auxLabels) {
    ...
    double calcSn(double u) {
        ...
        int j;
        // q = modular constant
        double q = Math.exp(-Math.PI * Kprime / K);
        double v = Math.PI * .5 * u / K;
        for (j = 0;; j++) {
    ...
    public Complex getSPole(int i, double wc) {
        ...
        return  new Complex(rootR[i + 1] * tanwc, rootI[i + 1] * tanwc);
    }

    protected Complex getEllipticZero(int i, double wc) {
        double tanwc = Math.tan(wc * .5);
        Complex result = new Complex(0, zeros[i / 2] * tanwc);
        if ((i & 1) == 1) {
            result.setIm(-result.getIm());
        }
